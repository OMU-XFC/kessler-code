...
class SaveModelCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        if self.n_calls % self.save_freq == 0:
            model_path = os.path.join(self.save_path, f'model5_{self.n_calls}.zip')
...
def train():
    #ここで選択するシナリオはダミー
    kessler_env = Monitor(KesslerEnv(scenario=wall_right_wrap_2))
...
def run():
    ...
    scenario = accuracy_test_6
    score, perf_list = kessler_game.run(scenario=scenario, controllers=[SuperDummyController(), Controller()], stop_on_no_asteroids=True)
...
class SuperDummyController(KesslerController):
    def __init__(self):
        self.model = PPO.load("wcci-out/model3_60000000.zip")
    ...
    def actions(self, ship_state: Dict, game_state: Dict) -> Tuple[float, float, bool, bool]:
        ...
        print(len(game_state['asteroids']))
        print(ship_state)
        return thrust * THRUST_SCALE, turn * TURN_SCALE, fire_bullet, mine > 0.0
