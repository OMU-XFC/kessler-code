...
def weighted_average_similar_outs(rule_array, out_array, x):
    ...
    # 一致している個数を数える
    differences = np.sum((X12 - x) ** 2, axis=1)
    # 類似度が高いものを取り出す
    dif_index = np.argsort(differences)

    # 上位k個の類似度のインデックスを取得
    top_k_indices = dif_index[:5]
...
if __name__ == "__main__":
    #NNによる制御の入出力
    file_path = 'inout/Scenarios_full22.txt'
    ...
    file_path = 'inout/Scenarios_full22_fuzzcon.txt'

    for x in X:
        line = actions_from_obs(x)
